/*
 Many of the states have different date formats. This file converts those that are in differeny but valid formats to
 a single format  MM/DD/YYYY

 NOTE: Preserve the order of these updates, some depend on previous format conversions that convert single digit days or months to double digits for proper substring calculations
